...
export default function QuestionTwo({
  ...
  /* notify parent whenever we have / lose a document */
  useEffect(() => {
    cbRef.current?.(doc ? doc.file : null);
  }, [doc]);
  ...
  const handleUpload = (file: File | null) => {
    if (!file) return;
    if (doc) URL.revokeObjectURL(doc.url); // cleanup old preview
    ...
    <>
      <section className="mx-auto max-w-2xl px-4">
        <h2 className="mb-2 text-2xl font-extrabold">
          Upload your deed or title document
        </h2>
        <p className="mb-10 text-sm text-gray-600">
          This helps us verify legal ownership.
        </p>
